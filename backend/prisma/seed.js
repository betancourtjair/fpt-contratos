...
async function main() {
  ...
  await withTransaction(async (client) => {
    ...
    // esNda / esServicios activan, en el formulario de "Nueva solicitud", los campos propios
    // de NDA y de prestación de servicios (ver contrato_nda_detalles / contrato_servicios_detalles
    // y frontend/src/components/ContratoForm.jsx). Si el tipo ya existía en la BD (instalación
    // no nueva), el ON CONFLICT también actualiza la bandera para que quede prendida.
    const tiposDeseados = [
      ...
      { nombre: 'Confidencialidad (NDA)', descripcion: 'Acuerdos de confidencialidad.', esNda: true },
      { nombre: 'Servicios Profesionales', descripcion: 'Contratación de servicios profesionales/consultoría.', esServicios: true },
    ];
    const tipoIdPorNombre = {};
    for (const tipo of tiposDeseados) {
      const { rows } = await client.query(
        `INSERT INTO tipos_contrato (nombre, descripcion, es_nda, es_servicios)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (nombre) DO UPDATE SET
           descripcion = EXCLUDED.descripcion,
           es_nda = EXCLUDED.es_nda,
           es_servicios = EXCLUDED.es_servicios
         RETURNING id, nombre`,
        [tipo.nombre, tipo.descripcion, Boolean(tipo.esNda), Boolean(tipo.esServicios)]
      );
